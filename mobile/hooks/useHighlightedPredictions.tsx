...
import { FixtureDoc, HighlightItem, PredictionDoc } from "@/types";
import { query, collection, where, orderBy, limit, Unsubscribe, onSnapshot, documentId, getDocs } from "firebase/firestore";
...
export const useHighlightedPredictions = () => {
  ...
  const fixtureIds = useMemo(() => preds.map((p) => String(p.fixtureId)), [preds]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        if (!cancelled) setError(e?.message ?? "Failed to load fixtures for highlights");
      }
    }
  ...
  const items: HighlightItem[] = useMemo(() => {
    return preds
      .map((p) => {
        ...
        if (!fx) return null;
        return { fixtureId: String(p.fixtureId), fixture: fx, prediction: p };
      })
